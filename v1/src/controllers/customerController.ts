...
class Customer extends BaseController {
  ...
  update = (req: Request, res: Response) => {
    console.log("req comes in update method of customer controller", req.body);
      ...
      this.service
        ...
        .then((existingCustomer: ICustomer) => {
          ...
          if (existingCustomer.name === name) {
            this.service
              .update(user.id, existingCustomer._id.toString(), req.body)
  ...
  getCustomers = (req: Request, res: Response) => {
    try {
      this.service.baseModel.find({user_id: (req as any).user.id, deleted: false}, '_id name').then((customers: ICustomer[]) => {
  ...
  list = async(req: Request, res: Response) => {
    ...
    console.log("pipeline for listing customers", pipeline);
    let list = await this.service.baseModel.aggregate(pipeline).exec();
    console.log("list of customers after aggregation", list);
  ...
  uploadCustomers = async(req: Request, res: Response) => {
    try {
      ...
      const customers = rows.map((row: any, index) => {
      ...
      }).filter((customer: any) => !customer.error);
      const inserted = [];
